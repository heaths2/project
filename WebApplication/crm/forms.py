...
class CompanyForm(forms.ModelForm):
    ...
    business_conditions = forms.ChoiceField(
        choices=BUSINESS_CONDITIONS_CHOICES,
        initial={'business_conditions': '업태 선택'},
        label='업태',
        widget=forms.Select(attrs={'class': 'form-control', 'name': 'business_conditions', 'placeholder': '업태', 'type': 'text'}),
        required=False,
    ...
    category_of_business = forms.ChoiceField(
        choices=CATEGORY_OF_BUSINESS_CHOICES,
        initial={'category_of_business': '업종 선택'},
        label='업종',
...
class CustomerForm(forms.ModelForm):
    ...
    def clean_classification(self):
        ...
        queryset = Customer.objects.filter(classification=classification)
        if queryset.exists():
	        raise forms.ValidationError('다른 구분을 입력하시오.')
        return classification

    def clean_customer(self):
        customer = self.cleaned_data.get('customer')
        queryset = Customer.objects.filter(customer=customer)
        if queryset.exists():
	        raise forms.ValidationError('다른 담당자를 입력하시오.')
        return customer

    def clean_telephone(self):
        telephone = self.cleaned_data.get('telephone')
        queryset = Customer.objects.filter(telephone=telephone)
        if queryset.exists():
	        raise forms.ValidationError('다른 전화번호를 입력하시오.')
        return telephone

    def clean_cellphone(self):
        cellphone = self.cleaned_data.get('cellphone')
        queryset = Customer.objects.filter(cellphone=cellphone)
        if queryset.exists():
	        raise forms.ValidationError('다른 휴대폰번호를 입력하시오.')
        return cellphone
    ...
    def clean_email(self):
        ...
        if queryset.exists():
	        raise forms.ValidationError('다른 이메일을 입력하시오.')
        return email
...
class ContractForm(forms.ModelForm):
    ...
    contract_type = forms.ChoiceField(
        ...
        initial={'contract_type': '유지보수 구분'},
        label='유지보수 구분',
        widget=forms.Select(attrs={'class': 'form-control', 'name': 'contract_type', 'placeholder': '유지보수 구분', 'type': 'text'}),
        required=False,
        error_messages={'required': '유지보수 구분을 선택하시오.'},
    )
    contract_period_start = forms.DateField(label='유지보수 시작 기간',
        widget=forms.DateInput(attrs={'class': 'form-control', 'name': 'contract_period_start', 'placeholder': '유지보수 시작 기간', 'type': 'date'}),
        required=False,
        error_messages={'required': '유지보수 시작 기간을 입력하시오.'},
    )
    contract_period_end = forms.DateField(label='유지보수 종료 기간',
        widget=forms.DateInput(attrs={'class': 'form-control', 'name': 'contract_period_end', 'placeholder': '유지보수 종료 기간', 'type': 'date'}),
        required=False,
        error_messages={'required': '유지보수 종료 기간을 선택하시오.'},
    )
    note = forms.CharField(label='비  고',
        widget=forms.TextInput(attrs={'class': 'form-control', 'name': 'note', 'placeholder': '비  고', 'type': 'text', }),
        required=False,
